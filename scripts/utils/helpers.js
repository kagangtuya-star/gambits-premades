...
export async function gmUpdateDisposition({ tokenUuid, disposition }) {
  if (
    !tokenUuid ||
    (disposition !== 0 && disposition !== 1 && disposition !== -1)
  )
    return;
...
export async function gmToggleStatus({ tokenUuid, status, active }) {
  ...
  let token = await fromUuid(`${tokenUuid}`);
  if (token)
    await token.actor.toggleStatusEffect(status, {
      active: active,
      overlay: false,
    });
}
      ...
      buttons: [
        {
          ...
          callback: async (event, button, dialog) => {
            ...
            ui.notifications.info(
              `You used your once per long rest option to initiate ${item.name} and did not use a spell slot`
            );
...
export async function closeDialogById({ dialogId }) {
  if (
    ui.activeWindow &&
    ui.activeWindow.dialogState &&
    ui.activeWindow.dialogState.dialogId === dialogId
  ) {
    ui.activeWindow.dialogState.programmaticallyClosed = true;
    ui.activeWindow.close();
  } else {
    let dialog = Object.values(ui.windows).find(
      (d) => d.dialogState && d.dialogState.dialogId === dialogId
    );
    if (dialog) {
...
export async function handleDialogPromises({ userDialogArgs, gmDialogArgs }) {
  ...
  let userDialogPromise = game.gps.socket.executeAsUser(
    "process3rdPartyReactionDialog",
    userDialogArgs.browserUser,
    userDialogArgs
  );
  let gmDialogPromise = game.gps.socket.executeAsUser(
    "process3rdPartyReactionDialog",
    getPrimaryGM(),
    gmDialogArgs
  );
...
export function findValidTokens({
  initiatingToken,
  targetedToken,
  itemName,
  itemType,
  ...
  reactionCheck,
  sightCheck,
  sightCheckType = "enemy",
  rangeCheck,
  rangeTotal,
  dispositionCheck,
  dispositionCheckType,
  workflowType,
  workflowCombat,
  gpsUuid,
  sourceRules = "2014",
  ...
  if (gpsUuid)
    checkItem = targetedToken?.actor?.items?.find(
      (i) => i.flags["gambits-premades"]?.gpsUuid === gpsUuid
    );
  else
    checkItem = targetedToken?.actor?.items?.find(
      (i) => i.name.toLowerCase() === cleanedItemName
    );

  let debugEnabled = MidiQOL.safeGetGameSetting(
    "gambits-premades",
    "debugEnabled"
  );
  let workflowNonCombat = MidiQOL.safeGetGameSetting(
    "gambits-premades",
    "enable3prNoCombat"
  );
  ...
  if (!workflowCombat || workflowNonCombat) {
    ...
    validTokens = game.combat?.combatants
      ?.map((combatant) => canvas.tokens.get(combatant.tokenId))
      .filter((t) => filterToken(t));
  }
  ...
  function filterToken(t) {
    ...
    if (!t.actor) {
      if (debugEnabled)
        console.error(
          `${itemName} for ${
            t.actor ? t.actor.name : "Unknown Actor"
          } failed at invalid token actor on canvas`
        );
      return;
    }
    let checkItem;
    ...
    let hasEffectOrigin = t?.actor?.appliedEffects.some((effect) =>
      effectNamesOrigin.includes(effect.name)
    );
    let measuredDistance =
      dispositionCheckType === "ally" || dispositionCheckType === "enemyAlly"
        ? MidiQOL.computeDistance(targetedToken, t, {
            wallsBlock: true,
            includeCover: true,
          })
        : MidiQOL.computeDistance(initiatingToken, t, {
            wallsBlock: true,
            includeCover: true,
          });
    ...
    if (!checkItem && sourceRules === "2024") {
      if (debugEnabled)
        console.error(
          `${itemName} 2024 for ${t.actor.name} failed at check if reaction item exists`
        );
      return;
    } else if (!checkItem) {
      if (debugEnabled)
        console.error(
          `${itemName} for ${t.actor.name} failed at check if reaction item exists`
        );
      return;
    }
    ...
    // Check if the tokens reaction already used
      ...
      if (debugEnabled)
        console.error(
          `${itemName} for ${t.actor.name} failed at reaction available`
        );
      return;
    }
    ...
    // Check if the token is incapacitated
      ...
      if (debugEnabled)
        console.error(
          `${itemName} for ${t.actor.name} failed at is incapacitated`
        );
      return;
    }
    ...
    // Check if the token is the initiating token or not a qualifying token disposition
      ...
      dispositionCheck &&
      (((dispositionCheckType === "enemy" ||
        dispositionCheckType === "enemyAlly") &&
        t.document.disposition === initiatingToken.document.disposition) ||
        (dispositionCheckType === "ally" &&
          t.document.disposition !== initiatingToken.document.disposition))
    ) {
      if (debugEnabled)
        console.error(
          `${itemName} for ${t.actor.name} failed at token disposition check`
        );
      return;
    }
    ...
    // Check if token can see initiating token
      ...
      sightCheck &&
      ((sightCheckType === "ally" && !MidiQOL.canSee(t, targetedToken)) ||
        (sightCheckType === "enemy" && !MidiQOL.canSee(t, initiatingToken)))
    ) {
      if (debugEnabled)
        console.error(`${itemName} for ${t.actor.name} failed at sight check`);
      return;
    }
    ...
    // Check if token is under an effect preventing reactions
      ...
      if (debugEnabled)
        console.error(
          `${itemName} for ${t.actor.name} failed at spell effect preventing reaction`
        );
      return;
    }
    ...
    // Check if token is within range
      ...
      rangeCheck &&
      (measuredDistance === -1 || measuredDistance > range)
    ) {
      if (debugEnabled)
        console.error(`${itemName} for ${t.actor.name} failed at range check`);
      return;
    }
    ...
    if (itemType === "spell") {
      ...
      if (
        checkType === "prepared" &&
        checkItem?.system?.preparation?.prepared === false
      )
        return false;
      ...
      if (!hasSpellSlots) {
        if (debugEnabled)
          console.error(
            `${itemName} for ${t.actor.name} failed at check valid spell slots/preparation`
          );
        return false;
      }
    }
    ...
    if (itemType === "feature") {
      let itemNames;
      if (itemChecked)
        itemNames = itemChecked.map((item) => item.toLowerCase());
      else itemNames = [checkItem.name.toLowerCase()];
      let resourceExistsWithValue;
      let itemExistsWithValue;

      if (itemNames.includes("legres"))
        resourceExistsWithValue =
          t.actor.system.resources.legres.value !== 0 ? true : false;
      else
        itemExistsWithValue = t.actor.items.some(
          (i) =>
            (itemNames.includes(i.name.toLowerCase()) ||
              itemNames.includes(i.identifier.toLowerCase())) &&
            i.system.uses?.spent < i.system.uses?.max
        );

      if (!itemExistsWithValue && !resourceExistsWithValue) {
        if (debugEnabled)
          console.error(
            `${itemName} for ${t.actor.name} failed at check valid feature item/resource uses`
          );
        return;
      }
    }
    ...
    if (itemType === "item") {
      ...
      let itemExists = t.actor.items.some(
        (i) =>
          itemNames.includes(i.identifier.toLowerCase()) ||
          itemNames.includes(i.name.toLowerCase()) ||
          itemNames.includes(i.system.actionType?.toLowerCase())
      );
      if (!itemExists)
        if (!itemExists) {
          if (debugEnabled)
            console.error(
              `${itemName} for ${t.actor.name} failed at valid item supporting feature`
            );
          return;
        }
    }
    ...
    if (debugEnabled) {
      console.warn(
        `%c${itemName} for ${t.actor.name} Reaction Validation Passed`,
        "font-weight: bold"
      );
    }
...
export function findValidToken({
  ...
  if (workflowCombat && !workflowNonCombat) {
    ...
    if (!combat) {
      if (debugEnabled)
        console.error(`${itemName} failed at check if combat active`);
      return false;
    }
  }
  ...
  if (!targetedToken) {
    if (debugEnabled)
      console.error(`${itemName} failed no targetedToken found`);
    return false;
  }
  ...
  if (!initiatingToken) {
    if (debugEnabled)
      console.error(`${itemName} failed no initiatingToken found`);
    return false;
  }
  ...
  if (!targetedToken.actor) {
    if (debugEnabled)
      console.error(
        `${itemName} for ${
          targetedToken.actor ? targetedToken.actor.name : "Unknown Actor"
        } failed at invalid token actor on canvas`
      );
    return false;
  }
  ...
  let hasEffectOrigin = targetedToken?.actor?.appliedEffects.some((effect) =>
    effectNamesOrigin.includes(effect.name)
  );
  let measuredDistance = MidiQOL.computeDistance(
    initiatingToken,
    targetedToken,
    { wallsBlock: true, includeCover: true }
  );
  let range = game.gps.convertFromFeet({ range: rangeTotal });
  ...
  // Check if the tokens reaction already used
    ...
    if (debugEnabled)
      console.error(
        `${itemName} for ${targetedToken.actor.name} failed at reaction available`
      );
    return false;
  }
  ...
  // Check if the token is incapacitated
    ...
    if (debugEnabled)
      console.error(
        `${itemName} for ${targetedToken.actor.name} failed at is incapacitated`
      );
    return false;
  }
  ...
  // Check if the token is the initiating token or not a qualifying token disposition
    ...
    dispositionCheck &&
    ((targetedToken.id === initiatingToken.id && workflowType === "attack") ||
      ((dispositionCheckType === "enemy" ||
        dispositionCheckType === "enemyAlly") &&
        targetedToken.document.disposition ===
          initiatingToken.document.disposition) ||
      (dispositionCheckType === "ally" &&
        targetedToken.document.disposition !==
          initiatingToken.document.disposition))
  ) {
    if (debugEnabled)
      console.error(
        `${itemName} for ${targetedToken.actor.name} failed at token disposition check`
      );
    return false;
  }
  ...
  // Check if token can see initiating token
    ...
    sightCheck &&
    ((sightCheckType === "ally" &&
      !MidiQOL.canSee(initiatingToken, targetedToken)) ||
      (sightCheckType === "enemy" &&
        !MidiQOL.canSee(targetedToken, initiatingToken)))
  ) {
    if (debugEnabled)
      console.error(
        `${itemName} for ${targetedToken.actor.name} failed at sight check`
      );
    return false;
  }
  ...
  // Check if token is under an effect preventing reactions
    ...
    if (debugEnabled)
      console.error(
        `${itemName} for ${targetedToken.actor.name} failed at spell effect preventing reaction`
      );
    return false;
  }
  ...
  // Check if token is within range
    ...
    if (debugEnabled)
      console.error(
        `${itemName} for ${targetedToken.actor.name} failed at range check`
      );
    return false;
  }
  ...
  if (itemType === "spell") {
    ...
    if (checkType === "prepared" || checkType === "always") {
      for (let level = spellLevel; level <= 9; level++) {
        let spellSlot =
          targetedToken.actor.system.spells[`spell${level}`].value;
        if (spellSlot > 0) {
          hasSpellSlots = true;
          break;
        }
    ...
    if (!hasSpellSlots) {
      if (debugEnabled)
        console.error(
          `${itemName} for ${targetedToken.actor.name} failed at check valid spell slots/preparation`
        );
      return false;
    }
  }
  ...
  if (itemType === "feature") {
    ...
    if (itemNames.includes("legres"))
      resourceExistsWithValue =
        targetedToken.actor.system.resources.legres.value !== 0 ? true : false;
    else
      itemExistsWithValue = targetedToken.actor.items.some(
        (i) =>
          (itemNames.includes(i.name.toLowerCase()) ||
            itemNames.includes(i.identifier.toLowerCase())) &&
          i.system.uses?.spent < i.system.uses?.max
    ...
    if (!itemExistsWithValue && !resourceExistsWithValue) {
      if (debugEnabled)
        console.error(
          `${itemName} for ${targetedToken.actor.name} failed at check valid feature item/resource uses`
        );
      return false;
    }
  }
  ...
  if (itemType === "item") {
    const itemNames = itemChecked.map((item) => item.toLowerCase());
    let itemExists = targetedToken.actor.items.some(
      (i) =>
        itemNames.includes(i.identifier.toLowerCase()) ||
        itemNames.includes(i.name.toLowerCase()) ||
        itemNames.includes(i.system.actionType?.toLowerCase())
    ...
    if (!itemExists) {
      if (debugEnabled)
        console.error(
          `${itemName} for ${targetedToken.actor.name} failed at valid item supporting feature`
        );
      return false;
    }
  }
  ...
  if (debugEnabled) {
    console.warn(
      `%c${itemName} for ${targetedToken.actor.name} Reaction Validation Passed`,
      "font-weight: bold"
    );
  }
...
export async function process3rdPartyReactionDialog({
  ...
  function attachEventListeners(dialog, animate) {
    ...
    const pauseButton = dialogElement?.querySelector(
      `#pauseButton_${dialogId}`
    );
    ...
    function handleCheckboxChange(event) {
      const checkedBoxes = dialogElement.querySelectorAll(
        "input.enemy-tokens:checked"
      );

      if (checkedBoxes.length > numTargets) {
        event.target.checked = false;
        ui.notifications.warn(
          `You can only select up to ${numTargets} targets.`
        );
      }
    }
  ...
  await foundry.applications.api.DialogV2.wait({
    ...
    buttons: [
      {
        ...
        callback: async (event, button, dialog) => {
          ...
          if (source && source === "user" && type === "multiDialog")
            await game.gps.socket.executeAsUser(
              "closeDialogById",
              getPrimaryGM(),
              { dialogId: dialogId }
            );
          ...
          let enemyTokenUuid =
            button.form?.elements["enemy-token"]?.value ?? false;
          let allyTokenUuid =
            button.form?.elements["ally-token"]?.value ?? false;
          ...
          let damageChosen = [];
          let damageListItems = document?.querySelectorAll(
            `#damage-list li .damage-type`
          );
          if (damageListItems?.length > 0) {
            damageListItems.forEach((item) =>
              damageChosen.push(item.textContent.trim())
            );
          }

          let selectedItemUuid =
            button.form?.elements[`item-select_${dialogId}`]?.value ?? false;
          let favoriteCheck =
            button.form?.elements["gps-favorite-checkbox"]?.checked ?? false;
          let genericCheck =
            button.form?.elements["gps-checkbox"]?.checked ?? false;

          let enemyTokenUuids =
            document?.querySelectorAll("input.enemy-tokens:checked") ?? false;
          if (enemyTokenUuids)
            enemyTokenUuids = Array.from(enemyTokenUuids).map(
    ...
    render: (event) => {
      ...
      dialog.updateTimer = function updateTimer(newTimeLeft, paused) {
        ...
        if (game.settings.get("gambits-premades", "enableTimerFullAnim"))
          useFullTitleBar = true;
        this.timeLeft = newTimeLeft;
        this.isPaused = paused;
        const dialogElement = dialog?.element;
        const pauseIcon = dialogElement?.querySelector(
          `#pauseIcon_${dialogId}`
        );
        const pauseButton = dialogElement?.querySelector(
          `#pauseButton_${dialogId}`
        ...
        if (titleElement) {
          titleElement.textContent = `${dialogTitle} - ${Math.ceil(
            this.timeLeft
          )}s`;
        }
        ...
        let dialogColors = getDialogColors({
          type,
          source,
        ...
        const selectElements =
          dialogElement?.querySelectorAll(".gps-dialog-select") || [];
        selectElements.forEach((el) => {
          el.style.backgroundColor = dialogColors.selectColor;
        });

        const optionElements =
          dialogElement?.querySelectorAll(".gps-dialog-option") || [];
        ...
        if (pauseIcon && pauseButton) {
          ...
          pauseButton.style.backgroundColor = this.isPaused
            ? dialogColors.pauseColor
            : "";
        }
      };
      requestAnimationFrame(dialog.animate);
    ...
    close: async (event, dialog) => {
      ...
      if (
        ...
        source === "user" &&
        type === "multiDialog" &&
        !dialog.dialogState.programmaticallyClosed
      )
        await game.gps.socket.executeAsUser("closeDialogById", getPrimaryGM(), {
          dialogId: dialogId,
        });
      else if (
        source &&
        ...
        await game.gps.socket.executeAsUser("closeDialogById", browserUser, {
          dialogId: dialogId,
        });
    },
    rejectClose: false,
...
export async function moveTokenByOriginPoint({
  originX,
  originY,
  targetUuid,
  distance,
  ...
  if (!distance || isNaN(distance))
    return console.log("No valid distance to move");
  if (!originX || !originY)
    return console.log(
      "No valid origin x/y coordinate given, for a token object this value should be token.center.x/token.center.y"
    );
  ...
  let measuredCost;
  let finalX =
    target.center.x + Math.cos(rayAngle) * (allowedDistance * pixelsPerFoot);
  let finalY =
    target.center.y + Math.sin(rayAngle) * (allowedDistance * pixelsPerFoot);

  let collisionDetected = CONFIG.Canvas.polygonBackends.move.testCollision(
    target.center,
    new PIXI.Point(finalX, finalY),
    { type: "move", mode: "any" }
  );
  ...
  if (!collisionDetected) {
    if (canvas.scene.grid.type === 1) {
      while (true) {
        let intendedX =
          target.center.x +
          Math.cos(rayAngle) * (allowedDistance * pixelsPerFoot);
        let intendedY =
          target.center.y +
          Math.sin(rayAngle) * (allowedDistance * pixelsPerFoot);
...
export async function moveTokenByCardinal({ targetUuid, distance, direction }) {
  ...
  if (!directions.includes(direction))
    return console.log(
      "No valid direction to move (Valid Options: 'North', 'South', 'East', 'West', 'Northwest', 'Northeast', 'Southwest', 'Southeast')"
    );
  ...
  let measuredCost;
  let finalX =
    target.center.x + Math.cos(angle) * (allowedDistance * pixelsPerFoot);
  let finalY =
    target.center.y + Math.sin(angle) * (allowedDistance * pixelsPerFoot);
...
export async function replaceChatCard({
  actorUuid,
  itemUuid,
  chatContent,
  rollData,
  ...
  game.messages.reduce((list, message) => {
    if (
      message.flags.dnd5e?.item.id === item._id &&
      message.speaker.token === token.document.id
    )
      msgHistory.push(message.id);
  }, msgHistory);
  ...
  if (chatMessage) {
    ...
    let itemContent = await renderTemplate(
...
export function validateRegionMovement({
  regionScenario,
  regionStatus,
  regionUuid,
  tokenUuid,
  ...
  if (regionScenario === "tokenStatusChanged" && regionStatus) {
    ...
    return (validatedRegionMovement = {
      validRegionMovement: false,
      validReroute: false,
    });
  } else if (regionScenario === "tokenStatusChanged" && !regionStatus)
    return (validatedRegionMovement = {
      validRegionMovement: false,
      validReroute: false,
    });
  ...
  if (regionScenario === "tokenExit") {
    ...
    const forcedMovement = regionTokenStates.get(
      `${regionId}-${tokenId}-forcedMovement`
    );
    ...
    if (forcedMovement)
      regionTokenStates.delete(`${regionId}-${tokenId}-forcedMovement`);
    if (statusChanged)
      regionTokenStates.delete(`${regionId}-${tokenId}-statuschanged`);

    if (forcedMovement || statusChanged)
      return (validatedRegionMovement = {
        validRegionMovement: false,
        validReroute: false,
      });
...
export function getPrimaryGM() {
  const primaryGMId = MidiQOL.safeGetGameSetting(
    "gambits-premades",
    `primaryGM`
  );
  const primaryGM = game.users.find(
    (user) => user.id === primaryGMId && user.active && user.isGM
  );
...
export function getCprConfig({ itemUuid }) {
  if (!game.modules.get("chris-premades")?.active)
    return { animEnabled: true, animColor: null };
  ...
  if (animEnabled === true || animEnabled === false)
    return { animEnabled: animEnabled, animColor: animColor };
  else return { animEnabled: true, animColor: animColor };
}

export async function remoteCompleteItemUse({
  itemUuid,
  actorUuid,
  options,
  isWeapon = false,
  ...
  if (isWeapon) {
    ...
    configSettings = foundry.utils.duplicate(
    ...
    if (originalCheckRange !== "none") {
      await game.gps.socket.executeAsUser(
        "gpsUpdateMidiRange",
        game.gps.getPrimaryGM(),
        {
          configSettings: configSettings,
          turnOff: true,
          originalCheckRange: originalCheckRange,
        }
      );
    }
  }
  ...
  return {
    castLevel: remoteCIU?.castData?.castLevel,
    baseLevel: remoteCIU?.castData?.baseLevel,
    itemType: remoteCIU?.item?.system?.preparation?.mode,
    checkHits: checkHits,
  };
}
...
export async function gpsActivityUse({ itemUuid, identifier, targetUuid }) {
  ...
  if (!activity)
    return console.error(
      `Naughty Naughty: You've likely removed the identifier name from a ${item.name} automation activity and now it's nowhere to be found ¯\_(ツ)_/¯`
    );
  ...
  let activityUse = await MidiQOL.completeActivityUse(
    activity.uuid,
    options,
    {},
    {}
  ...
  return {
    saveRolls: activityUse?.saveRolls?.[0],
    failedSaves: { size: activityUse?.failedSaves?.size },
    failedSavesObject: activityUse?.failedSaves,
  };
}
...
export async function gpsUpdateMidiRange({
  configSettings,
  turnOff,
  originalCheckRange,
}) {
...
export function getDialogColors({ type, source, timeLeft, initialTimeLeft }) {
  ...
  const transitionPercentage =
    Math.max((transitionStart - progressPercentage) / transitionStart, 0) * 100;
  const rgbColor = getCssVarValue("--color-warm-2");
  const rgbColorSelect = getCssVarValue("--color-warm-3");
  let pauseColor = addAlphaToRgb(rgbColor, 0.6);
  let selectColor = addAlphaToRgb(rgbColorSelect, 0.2);
  ...
  if (hasCarolingianUI) {
    ...
    if (match) {
      ...
      redValue = Math.floor(
        baseR - (baseR - baseR * targetFactor) * (transitionPercentage / 100)
      );
      greenValue = Math.floor(
        baseG - (baseG - baseG * targetFactor) * (transitionPercentage / 100)
      );
      blueValue = Math.floor(
        baseB - (baseB - baseB * targetFactor) * (transitionPercentage / 100)
      );
...
export async function halfWeaponDamage({ workflow }) {
  ...
  workflow.damageRolls = await Promise.all(
    workflow.damageRolls.map(async (damageRoll) => {
      return await new CONFIG.Dice.DamageRoll(
        "floor((" + damageRoll.formula + ") / 2)",
